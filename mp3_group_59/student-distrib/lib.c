...
/* void clear_terminal_vmem(uint32_t terminal_vidmem_addr);
 * Inputs: void
 * Return Value: none
 * Function: Clears video memory for terminal */
void clear_terminal_vidmem(uint32_t terminal_vidmem_addr) {
    int32_t i;
    for (i = 0; i < NUM_ROWS * NUM_COLS; i++) {
        *(uint8_t *)(terminal_vidmem_addr + (i << 1)) = ' ';
        *(uint8_t *)(terminal_vidmem_addr + (i << 1) + 1) = ATTRIB;
    }
}
